...
vocab =['Bac', 'Ent', 'Esc','Lis','Pse', 'Sal', 'Sta','Sac','chr1','chr2','chr3','chr4','chr5','chr6','chr7','chr8','chr9','chr10',
'chr11','chr12','chr13','chr14','chr15','chr16','chr17','chr18','chr19','chr20','chr21','chr22','chrX','chrY','WH']

parser = argparse.ArgumentParser(description='PreTest_Predict')
parser.add_argument('--data_path', type=str, default='data/Human_2000_Sta_2000.npy', metavar='PATH',
                    help='your data files path (default:"data/Human_2000_Sta_2000.npy")')
...
def data_load(res_name, target_name,data_path, args):
    convert = label2int()
    ...
    human_data_num  = int(data_path.split("_")[1])
    target_data_num  = int(data_path.split("_")[3])
    targets= [1 for _ in range(human_data_num)] + [0 for _ in range(target_data_num)]
...
if __name__ == '__main__':
    ...
    data_path = Path(args.data_path).stem   
    target = data_path.split("_")[0]
    rest = data_path.split("_")[2]  

    target_model = model_load(args)

    rest_data   = data_load(rest, target,data_path, args)
    ...
    if os.path.exists(out_path) is not True:
      os.mkdir(out_path) 
    df.to_excel(out_path+'Human-output.xlsx', index=False)  
