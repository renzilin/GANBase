...
def load_symbol(config, symbol):
    ...
    if not isinstance(config, dict):
        ...
        # print('load_symbol config',config)
        ## {'global_norm': {'state_len': 4}, 'input': {'features': 1}, 'labels': {'labels': ['N', 'A', 'C', 'G', 'T']}, 'qscore': {'scale': 0.9356, 'bias': -0.1721}, 'model': {'package': 'bonito.crf'}, 'encoder': {'scale': 5.0, 'rnn_type': 'lstm', 'winlen': 19, 'features': 384, 'activation': 'swish', 'stride': 5, 'blank_score': 2.0}, 'basecaller': {'batchsize': 512, 'chunksize': 10000, 'overlap': 500}}

    # print('load_symbol config_model',config['model']['package'])  # bonito.crf
    imported = import_module(config['model']['package'])
    # print('load_symbol imported',imported)    #  <module 'bonito.crf' from '/public/data1/zhangyx/Projects/related_work/bonito-0.7.3/bonito/crf/__init__.py'>
    # print('load_symbol-- ',getattr(imported, symbol))    #  <class 'bonito.crf.model.Model'>  
                                                        ## <function basecall at 0x7f4220117130>
...
def load_basecall_model(dirname, device, weights=None, half=None, chunksize=None, batchsize=None, overlap=None, quantize=False, use_koi=False):
    """
